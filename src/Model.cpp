...
namespace furSim {
	...
	std::shared_ptr<furSim::Mesh> Model::processMesh(aiMesh* mesh, const aiScene* scene, const glm::mat4 scaleMat)
	{
		// Data to fill
		std::vector<furSim::Mesh::Vertex> cpuVertexArray;
		...
		const int cpuVertexByteSize = sizeof(furSim::Mesh::Vertex) * numVertices;
		const int cpuIndexByteSize = sizeof(int) * cpuIndexArray.size();
		std::shared_ptr<furSim::Mesh> gpuMesh(new furSim::Mesh(textures, GL_TRIANGLES, GL_STATIC_DRAW, cpuVertexByteSize, cpuIndexByteSize, 0, cpuVertexArray, cpuIndexArray.size(), cpuIndexByteSize, &cpuIndexArray[0]));
