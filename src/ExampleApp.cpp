...
void ExampleApp::createTop(std::shared_ptr<basicgraphics::Mesh> &_mesh2) {
	...
	for (int i = 0; i < 361; i++) {
		Mesh::Vertex vert;
		vert.position = vec3(cos(radians((float)i)), 0.85f, sin(radians((float)i)));
		vert.normal = vec3(0, 0, 1);
...
void ExampleApp::createBot(std::shared_ptr<basicgraphics::Mesh> &_mesh2) {
	...
	for (int i = 361; i >= 0; i--) {
		Mesh::Vertex vert;
		vert.position = vec3(cos(radians((float)i)), yPos, sin(radians((float)i)));
		vert.normal = vec3(0, 0, 1);
		vert.texCoord0 = glm::vec2(.4, 0.5);
		cpuVertexArray.push_back(vert);
...
void ExampleApp::createSide(std::shared_ptr<basicgraphics::Mesh> &_mesh2) {
	...
	for (int i = 0; i < 361; i++) {
		if (i % 2 == 1) {
			Mesh::Vertex vert;
			vert.position = vec3(cos(radians((float)i)), -.85f, sin(radians((float)i)));
			vert.normal = vec3(0, 0, 1);
			vert.texCoord0 = glm::vec2(radians((float)i) / (2 * glm::pi<float>()), 1);
			cpuVertexArray.push_back(vert);
			cpuIndexArray.push_back(i+1);
			...
			vert.position = vec3(cos(radians((float)i)), 0.85f, sin(radians((float)i)));
			vert.normal = vec3(0, 0, 1);
			vert.texCoord0 = glm::vec2(radians((float)i)/(2* glm::pi<float>()), 0);
			cpuVertexArray.push_back(vert);
			cpuIndexArray.push_back(i+1);
