...
void main(void) {
	...
	currentLayer = 0;
	vec3 pos = vec3(interpSurfPosition) + interpSurfNormal * maxHairLength * currentLayer;

	vec4 worldPos = vec4(pos, 1) * model_mat;

	vec4 viewPos = worldPos * view_mat;

	gl_Position = viewPos * projection_mat;
}
